    ...
    void CreateNodes()
    {
        ...
        for (int x = scanStartX; x < scanFinishX; x++)
        {
            for (int y = scanStartY; y < scanFinishY; y++)
            {
                ...
                TileBase floorTileBase = floor.GetTile(new Vector3Int(x, y, 0)); //check if we have a floor tile at that world coords
                if (floorTileBase != null)
                {
                    bool check = Physics2D.OverlapCircle(new Vector3(x, y), 0.25f);
                    GameObject node = (GameObject)Instantiate(nodePrefab, new Vector3(x + gridBase.transform.position.x, y + gridBase.transform.position.y, 0), Quaternion.Euler(0, 0, 0));
                    if (check)
                    {
                    ...
                    // foreach (Tilemap obstacleTile in obstacleLayers)
                    // {
                    //     TileBase obstacleTileBase = obstacleTile.GetTile(new Vector3Int(x, y, 0));

                    //     if (obstacleTileBase != null)
                    //     {
                    //         foundObstacle = true;
                    //     }
                    ...
                    //     {
                    //         List<TileBase> neighbours = getNeighbouringTiles(x, y, obstacleTile);
                    //         foreach (TileBase tl in neighbours)
                    //         {
                    //             if (tl == null)
                    //             {
                    //                 Debug.LogWarning("tl is null!");
                    //             }
                    //             else
                    //             {
                    //                 foundObstacle = true;
